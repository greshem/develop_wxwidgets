...
void TestDialog::OnButtonSelect( wxCommandEvent& event )
{
    ...
    //wxFileDialog SaveDialog(this, _("Save As"), DefaultDir, wxEmptyString, _("Image file(*.IMG)|*.IMG|All file(*.*)|*.*"));
    wxFileDialog SaveDialog(this, _("Save As"), DefaultDir, wxEmptyString, _("Image file(*.cpp)|*.cpp|All file(*.*)|*.*"));
    if (SaveDialog.ShowModal() == wxID_OK)
    {
        ...
        if(wxFileExists(path))
		{
            	wxString OldName;
//			 int OldSize;
//          if(GetImageInfo(path, OldName, OldSize))
//			{
//          spDiskSize->SetValue(OldSize);
//          if(Operate == ADD && tDiskName->GetValue().IsEmpty())
//          {     
//              	tDiskName->SetValue(OldName);
//			}
		}
	}
}
